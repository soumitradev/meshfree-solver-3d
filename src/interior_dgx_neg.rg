...
task interior_dgx_neg(
  params: parameters,
  points: region(ispace(int1d), point),
  i: int
): double[5] where
reads (points.{x, y, z, tan1, tan2, nor, q, dq, qm, min_dist, xneg_nbhs, xneg_conn})
do
  ...
  if (params.limiter_flag == 1) then
    for k = 0, points[i].xneg_nbhs do
      var j = points[i].xneg_conn[k]
  ...
  for k = 0, points[i].xneg_nbhs do
    ...
    G_i = flux_Gwxn(params, tan1, tan2, nor, prim)
    ...
    prim = qtilde_to_primitive(qtilde)
    G_j = flux_Gwxn(params, tan1, tan2, nor, prim)
