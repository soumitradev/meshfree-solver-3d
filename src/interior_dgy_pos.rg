...
task interior_dgy_pos(
  params: parameters,
  points: region(ispace(int1d), point),
  i: int
): double[5] where
reads (points.{x, y, z, tan1, tan2, nor, q, dq, qm, min_dist, ypos_nbhs, ypos_conn})
do
  ...
  if (params.limiter_flag == 1) then
    for k = 0, points[i].ypos_nbhs do
      var j = points[i].ypos_conn[k]
  ...
  for k = 0, points[i].ypos_nbhs do
    ...
    G_i = flux_Gwyp(params, tan1, tan2, nor, prim)
    ...
    prim = qtilde_to_primitive(qtilde)
    G_j = flux_Gwyp(params, tan1, tan2, nor, prim)
