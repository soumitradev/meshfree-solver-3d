...
task interior_dgz_neg(
  params: parameters,
  points: region(ispace(int1d), point),
  i: int
): double[5] where
reads (points.{x, y, z, tan1, tan2, nor, q, dq, qm, min_dist, zneg_nbhs, zneg_conn})
do
  ...
  if (params.limiter_flag == 1) then
    for k = 0, points[i].zneg_nbhs do
      var j = points[i].zneg_conn[k]
  ...
  for k = 0, points[i].zneg_nbhs do
    ...
    G_i = flux_Gzn(params, tan1, tan2, nor, prim)
    ...
    prim = qtilde_to_primitive(qtilde)
    G_j = flux_Gzn(params, tan1, tan2, nor, prim)
