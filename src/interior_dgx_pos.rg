...
task interior_dgx_pos(
  params: parameters,
  points: region(ispace(int1d), point),
  i: int
): double[5] where
reads (points.{x, y, z, tan1, tan2, nor, q, dq, qm, min_dist, xpos_nbhs, xpos_conn})
do
  ...
  if (params.limiter_flag == 1) then
    for k = 0, points[i].xpos_nbhs do
      var j = points[i].xpos_conn[k]
  ...
  for k = 0, points[i].xpos_nbhs do
    ...
    G_i = flux_Gwxp(params, tan1, tan2, nor, prim)
    ...
    prim = qtilde_to_primitive(qtilde)
    G_j = flux_Gwxp(params, tan1, tan2, nor, prim)
